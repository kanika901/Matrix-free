...
PetscErrorCode KSPMonitorJacobian(KSP ksp,PetscInt n,PetscReal rnorm,void *snes);
...
int main(int argc,char **argv)
{
  ...
  */
  ...
  {
    PetscBool ksp_monitor_jacobian = PETSC_FALSE;
    ierr = PetscOptionsGetBool(NULL,NULL,"-ksp_monitor_jacobian",&ksp_monitor_jacobian, NULL);CHKERRQ(ierr);
    if (ksp_monitor_jacobian) {
      ierr = KSPMonitorSet(ksp, &KSPMonitorJacobian, (void *)snes, NULL);CHKERRQ(ierr);
    }
  }
  ...
  {
    PetscViewer    viewer;
    PetscViewerASCIIOpen(PETSC_COMM_WORLD, "ex19_x0.m", &viewer);
  ...
  */
  if (user.draw_contours) {
    ...
    {
      PetscViewer    viewer;
      PetscViewerASCIIOpen(PETSC_COMM_WORLD, "ex19_xsol.m", &viewer);
      PetscViewerPushFormat(viewer, PETSC_VIEWER_ASCII_MATLAB);
      ierr = VecView(x,viewer);CHKERRQ(ierr);
      PetscViewerPopFormat(viewer);
      PetscViewerDestroy(&viewer);
...
PetscErrorCode KSPMonitorJacobian(KSP ksp, PetscInt n, PetscReal rnorm, void *snes)
{
  Mat            Amat;
  PetscErrorCode ierr;
  PetscInt       it_ksp, it_snes;
  char           fn[28];
  PetscViewer    viewer;

  ierr = KSPGetIterationNumber(ksp, &it_ksp);CHKERRQ(ierr);
  ierr = SNESGetIterationNumber((SNES) snes, &it_snes);CHKERRQ(ierr);

  if (it_ksp == 0) {
    sprintf(fn, "ex19_mat_snes%04d_ksp%04d.m", it_snes, it_ksp);
    PetscViewerASCIIOpen(PETSC_COMM_WORLD, fn, &viewer);
    PetscViewerPushFormat(viewer, PETSC_VIEWER_ASCII_MATLAB);
  ...
  //VecStrideNorm(Br,3,NORM_2,&norms[3]);
