...
def readFeatures(dirname='anamod_features'):
	...
	f = open(filename,'r')
        ...
        tmpdictstr.rstrip(',')
        tmpdictstr += '}'
        features[matrixname] = eval(tmpdictstr.replace('****',"'?'").replace('-nan',"'?'").replace('inf',"float('Inf')").replace('Inf',"float('Inf')").replace('NaN',"'?'"))
...
if __name__ == '__main__':
    ...
    print 'Perf data length::', len(perfdata.keys())
